...
class colour_search(object):
    ...
    def move_straight_to(self, target_x, target_y):
        ...
        self.robot_controller.publish()  
        print("straight move stopped")
        print("distance to target", sqrt(pow((target_x - self.x), 2) + pow((target_y - self.y), 2)))
        print("previous distance", prev_dist)
        print("intended pos ", target_x, target_y)
        print("current pos", self.x, self.y)
        
    # #returns correct bearing to turn to
    # def turn_by(self,target_yaw):
    #     if (self.theta_z + target_yaw) >= 360:
    #         self.turn_to(((self.theta_z + target_yaw) - 360))
    #     else:
    #         self.turn_to(((self.theta_z + target_yaw)))
    ...
    def turn_to(self, target_yaw, rotation):
        ...
        while not (((target_yaw - 2) < self.theta_z) and ((target_yaw + 2) > (self.theta_z))):
            ...
            self.robot_controller.publish()
        print("rotate stopped")
        print("facing ", self.theta_z)
        self.robot_controller.set_move_cmd(0.0, 0)
        self.robot_controller.publish()
